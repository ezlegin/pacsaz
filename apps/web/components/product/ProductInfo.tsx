...
const ProductInfo = ({
  ...
  const manufactureDimWidth = applyDimensionOffset(
    ...
    dimensionType,
    dimensionType === "inner"
      ? ptToMm(offset.width.inner)
      : ptToMm(offset.width.outer)
  );
  const manufactureDimLength = applyDimensionOffset(
    length,
    dimensionType,
    dimensionType === "inner"
      ? ptToMm(offset.length.inner)
      : ptToMm(offset.length.outer)
  );
        ...
        {dimensions.map(
          ({ label, value, key }) =>
            dimensionsType.includes(
              ...
              <div key={label} className="border w-1/2 p-2 rounded-2xl">
